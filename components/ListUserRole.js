                ...
                <Modal visible={modalOpen} animationType='slide'>
                    <ScrollView>
                        <View style={styles.modalContent}>
                            ...
                            <Subheading style={styles.textInput}>{t('UserRole.email')}: {modalData.email}</Subheading>
                            <Subheading style={styles.textInput}>{t('UserRole.currentRole')}: {modalData.role}</Subheading>
                            ...
                            <Button icon="check-circle" style={styles.button} mode="contained" onPress={()=>handleUpdateRole()} uppercase={false}>{t('UserRole.updateButton')}</Button>
                ...
                {userList.length!=0 ? userList.map((value) => (
                    ...
                    <Card>
                        <Card.Content>
                            ...
                            <Subheading>{t('UserRole.role')}: {value.role} </Subheading>
                            <Paragraph>{t('UserRole.area')}: {value.area}</Paragraph>
                        </Card.Content>
                        <Card.Actions>
                            <Button onPress={() => handleModal(value)}>{t('UserRole.modify')}</Button>
                        </Card.Actions>
                    </Card>
                    <Divider />
                    </View>
                )): <Card><Card.Title title={t('UserRole.noUser')}></Card.Title></Card>}
            </ScrollView>
        </View>
    )
}
